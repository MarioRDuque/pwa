...
export class ComprobanteElectronicoComprasComponent implements OnInit {
      ...
      if (this.comprasService.verificarPermiso(LS.ACCION_CREAR, this, true)) {
        ...
        if (form && form.valid && formularioTocado) {
          if (this.claveAcceso.length !== 49) {
            ...
            let rucProveedor = this.claveAcceso.substring(10, 23);
  ...
  despuesDeBuscarComprobanteElectronico(respuesta) {
    ...
    let objetoEnviar = { tipo: 'XML', proveedor: proveedor, fechaAutorizacion: fechaAutorizacion, fechaCaduca: fechaCaduca, fechaEmision: fechaEmision, invCompraTO: invCompraTO, anxCompraTO: anxCompraTO, minimo: minimo, maximo: maximo };
    this.enviarAccion.emit({ accion: LS.ACCION_CREAR, objetoEnviar: objetoEnviar });
    this.cargando = false;
  }
  ...
  cerrarFormularioProveedor(event) {
    if (event.invProveedorTO) {
      this.proveedor = new InvProveedorTO(event.invProveedorTO);
    }
