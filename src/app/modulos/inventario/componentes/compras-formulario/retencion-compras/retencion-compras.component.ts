...
export class RetencionComprasComponent implements OnInit {
  ...
  public isRequeridoAutorizacion: boolean = false;
  ...
  despuesDeObtenerDatosBasicosRetencionCreada(respuesta) {
    ...
    this.isRequeridoAutorizacion = this.anxCompraTO.compAutorizacion.length > 10 && this.invCompraTO.compDocumentoTipo !== LS.CODIGO_NOTA_CREDITO;
  ...
  generarOpcionesDetalleReembolso(rowIndex) {
    let permiso = (this.accion === LS.ACCION_MAYORIZAR || this.accion === LS.ACCION_CREAR);
    let permisoEliminar = (this.accion === LS.ACCION_MAYORIZAR || this.accion === LS.ACCION_CREAR);
